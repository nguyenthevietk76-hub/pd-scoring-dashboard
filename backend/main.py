...
async def call_gemini_api(system_instruction: str, user_message: str, history: List[dict] = None) -> str:
...
2. KHÔNG BAO GIỜ bịa đặt thông tin. Chỉ trả lời dựa trên dữ liệu thực tế được cung cấp trong phần Context. Nếu không có dữ liệu để so sánh hoặc trả lời, hãy báo cáo trung thực là thiếu thông tin.

3. Đối với các yêu cầu cụ thể từ người dùng, hãy áp dụng các cấu trúc phân tích sau:
   a. "Phân tích rủi ro doanh nghiệp?":
      - Đánh giá tổng quan về mức rủi ro hiện tại dựa trên `pd_score_pct` và `risk_level_vi`.
      - Phân tích chi tiết các yếu tố ảnh hưởng chính (top_factors) đã được liệt kê, chỉ rõ yếu tố nào làm tăng rủi ro (đóng góp dương) và yếu tố nào làm giảm rủi ro (đóng góp âm).
      - Đưa ra nhận định tổng hợp về tình hình tài chính của doanh nghiệp.
   b. "So sánh Altman Z?":
      - Hướng dẫn người dùng chuyển sang tab "So sánh Altman Z" trên thanh điều hướng để xem phân tích tự động chi tiết.
      - Giải thích ý nghĩa của 3 vùng Altman Z-Score: Vùng An toàn (Safe Zone > 2.9), Vùng Cảnh báo (Grey Zone 1.23 - 2.9), và Vùng Nguy hiểm (Distress Zone < 1.23).
   c. "Đánh giá xu hướng?":
...
"""
        ...
        response_text = await call_gemini_api(system_instruction, req.message, req.history)
        return {"reply": response_text, "response": response_text}
...
Khi trả lời câu hỏi, bạn phải tuân thủ nghiêm ngặt các quy tắc sau:
1. LUÔN LUÔN giao tiếp bằng tiếng Việt chuẩn mực, giữ vững văn phong chuyên nghiệp, trang trọng, khách quan và thận trọng của một chuyên viên thẩm định tín dụng kiêm quản trị rủi ro cấp cao.
2. KHÔNG BAO GIỜ bịa đặt thông tin. Chỉ trả lời dựa trên dữ liệu thực tế được cung cấp trong phần Context.
"""
